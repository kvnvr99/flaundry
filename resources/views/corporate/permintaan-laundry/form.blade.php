...
@component('component.form')
    @slot('action', !isset($data['detail']) ? route('permintaan-laundry-corporate.store') : route('permintaan-laundry-corporate.update'))
    @isset ($data['detail'])
        @slot('method','PUT')
    @else
        @slot('method','POST')
    ...
    <div class="form-group mb-3">
        ...
        <input type="hidden" value="{{ !isset($data['detail']) ? $data['info']->id : $data['detail'][0]->corporate_id }}" name="corporate_id">
    ...
    function ambil_layanan(){
        $('#modal-layanan').modal('show');
            ...
            $('#layanan-datatable').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: "{!! route('permintaan-laundry-corporate.get-data-layanan') !!}",
                    type: "POST",
                    dataType: "JSON"
                },
                columns: [
    ...
    function ambil_parfume(){
        $('#modal-parfume').modal('show');
            ...
            $('#state-saving-datatable').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: "{!! route('permintaan-laundry-corporate.get-data-parfume') !!}",
                    type: "POST",
                    dataType: "JSON"
                },
                columns: [
