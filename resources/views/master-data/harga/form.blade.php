...
@component('component.form')
    @slot('action', !isset($data['detail']) ? route('harga.store') : route('harga.update'))
    @isset ($data['detail'])
        @slot('method','PUT')
    @else
        @slot('method','POST')
    ...
    <div class="form-group mb-3">
        ...
        <input value="{{ !isset($data['detail']) ? old('harga') : old('harga', $data['detail']->harga) }}" type="text" name="harga" class="form-control mb-2 @error('harga') is-invalid @enderror" placeholder="harga" autocomplete="off" />
        @if($errors->has('harga'))
    ...
    <div class="form-group mb-3">
        ...
        <input value="{{ !isset($data['detail']) ? old('harga_member') : old('harga_member', $data['detail']->harga_member) }}" type="text" name="harga_member" class="form-control mb-2 @error('harga_member') is-invalid @enderror" placeholder="harga member" autocomplete="off" />
        @if($errors->has('harga_member'))
            <div class="text-danger"> {{ $errors->first('harga_member')}} </div>
        @endif
    </div>
